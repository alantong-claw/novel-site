...
async function ensureSingleCleanPage(context) {
  ...
  try {
    await keep.goto('about:blank', { waitUntil: 'load' });
  } catch {}
  return keep;
}
...
async function setupEditor(page) {
  ...
  await sleep(1000);
  if (!(await waitForCondition(async () => ({ ok: page.url().includes('/dashboard') }), { tries: 15, delayMs: 1000, label: 'dashboard' })).ok) throw new Error('did-not-reach-dashboard');
  ...
  }, { tries: 15, delayMs: 1000, label: 'posts' })).ok) throw new Error('did-not-reach-posts');
  ...
  }, { tries: 20, delayMs: 1000, label: 'create' })).ok) throw new Error('did-not-reach-create');
  ...
  }, { tries: 25, delayMs: 1000, label: 'editor' })).ok) throw new Error('did-not-reach-editor');
}
...
async function uploadImage(page, imagePath) {
  ...
  if (!clicked) throw new Error('image-button-not-found');
  await sleep(1200);
  ...
  }, { tries: 20, delayMs: 1000, label: 'image-upload' });
  if (!uploaded.ok) throw new Error('image-upload-not-confirmed');
}

async function publishOne(context, item) {
  const page = await ensureSingleCleanPage(context);
  ...
  const published = await waitForCondition(async () => {
    ...
    return {
      ok: url.startsWith('https://panel.pixnet.tw/posts') && body.includes(item.title),
      url,
      postUrl,
      body,
    };
  }, { tries: 25, delayMs: 1000, label: `publish-${item.num}` });

  return {
    num: item.num,
    title: item.title,
    success: published.ok,
    listUrl: published.url,
...
(async () => {
  const jsonPath = process.argv[2] || '/home/alantong/ai-work/tmp/pixnet-playwright-test/pixnet-republish-102-105-108.json';
  ...
  try {
    for (const item of items) {
      const result = await publishOne(context, item);
      results.push(result);
