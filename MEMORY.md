...
* **Investigation Workflow Rule (調查)**: For any investigation, first build a complete feature/overview checklist before drilling down into details. Use the checklist to confirm coverage and avoid missing key features, then proceed section-by-section. Continuously refine the investigation step over time.
...
* **Voice Output Formatting Rule**: In spoken/voice replies, strip or avoid `*` characters so TTS does not read out "star" / "星號" awkwardly. Do not rely on markdown emphasis in voice-facing text.
